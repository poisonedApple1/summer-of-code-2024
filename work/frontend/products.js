...
function getData_MakeList(offset){
    const get_all_products_url="http://127.0.0.1:5000/itemData";
...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    addButton.addEventListener('click',()=>{
        ...
        const signup_url="http://127.0.0.1:5000/item";
        fetch(signup_url,{
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    tableBody.addEventListener('click', function(event) {
      if (event.target.classList.contains('manageUpd')) {
        ...
        var item;
        const updateURLget="http://127.0.0.1:5000/item"
        fetch(updateURLget,{
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        const updateURL="http://127.0.0.1:5000/item/update"
      ...
      if (event.target.classList.contains('manageDel')) {
       ...
       delForm.addEventListener('submit',()=>{
            const urlDel="http://127.0.0.1:5000/item";
            fetch(urlDel,{
                method:'DELETE',
                headers:{'Content-Type':'application/json'},
                body:  JSON.stringify({Item_SKU})
